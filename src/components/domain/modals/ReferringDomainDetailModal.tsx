...
function getSpamBadge(score: number): { bg: string; text: string; label: string } {
    if (score <= 10) return { bg: "bg-utility-success-50", text: "text-utility-success-600", label: "Clean" };
    if (score <= 30) return { bg: "bg-utility-warning-50", text: "text-utility-warning-600", label: "Moderate" };
    return { bg: "bg-utility-error-50", text: "text-utility-error-600", label: "High Risk" };
}
            ...
            <div className="w-full max-w-3xl rounded-2xl bg-primary shadow-xl" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="max-h-[75vh] overflow-y-auto px-6 py-5 flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
                        <OverviewCard icon={LinkExternal01} label="Total Links" value={domain.linkCount.toString()} color="text-primary" />
                        <OverviewCard icon={ShieldTick} label="Dofollow" value={`${domain.dofollowPercent}%`} color="text-utility-success-600" />
                        <OverviewCard icon={BarChart01} label="Domain Rank" value={domain.avgDomainRank ? domain.avgDomainRank.toString() : "—"} color="text-primary" />
                        <OverviewCard icon={ShieldTick} label="Quality Score" value={`${domain.qualityScore}/100`} color={quality.text} />
                        <OverviewCard icon={ShieldTick} label="Spam Score" value={domain.avgSpamScore !== null ? `${domain.avgSpamScore}/100` : "—"} color={spam.text} />
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-1.5">
                                <span className="text-xs font-medium text-secondary">Spam Score</span>
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div className="rounded-lg border border-secondary p-4">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <span className="text-sm font-semibold text-primary">Link Breakdown</span>
                            </div>
                        ...
                        <div className="rounded-lg border border-secondary p-4">
                            ...
                            <div className="flex flex-col gap-2">
                                <div className="flex items-center justify-between">
                                    <span className="text-sm text-secondary">First Seen</span>
                                    <span className="text-sm font-medium text-primary">
                                        {domain.firstSeen ? new Date(domain.firstSeen).toLocaleDateString() : "—"}
                                    </span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="text-sm text-secondary">Last Seen</span>
                                    <span className="text-sm font-medium text-primary">
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <h4 className="text-sm font-semibold text-primary">
                                    Top Anchor Texts ({domain.topAnchors.length})
                                </h4>
                            </div>
