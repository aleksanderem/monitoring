...
const CHANNEL_LABELS: Record<string, { label: string; bg: string; text: string }> = {
    broken_link: { label: "Broken Link", bg: "bg-utility-error-50", text: "text-utility-error-600" },
    guest_post: { label: "Guest Post", bg: "bg-utility-brand-50", text: "text-utility-brand-600" },
    resource_page: { label: "Resource Page", bg: "bg-utility-blue-50", text: "text-utility-blue-600" },
    outreach: { label: "Outreach", bg: "bg-utility-warning-50", text: "text-utility-warning-600" },
    content_mention: { label: "Content Mention", bg: "bg-utility-success-50", text: "text-utility-success-600" },
};

const DIFFICULTY_BADGES: Record<string, { label: string; bg: string; text: string }> = {
...
const CHANNEL_FILTERS = [
    ...
    { label: "Content Mention", value: "content_mention" },
];
...
const STATUS_FILTERS = [
    { label: "All (excl. dismissed)", value: "all" },
...
export function LinkBuildingProspectsTable({ domainId }: LinkBuildingProspectsTableProps) {
    ...
    if (!prospects || prospects.length === 0) {
        return (
            <div className="flex flex-col gap-4 rounded-xl border border-secondary bg-primary p-6">
                <h3 className="text-md font-semibold text-primary">Link Building Prospects</h3>
                <div className="flex flex-col items-center justify-center py-8">
                    <p className="text-sm text-tertiary">No prospects generated yet</p>
                    <p className="mt-1 text-xs text-quaternary">Click &quot;Generate Report&quot; to analyze backlink gap opportunities</p>
                </div>
            </div>
        );
    }
            ...
            <div className="flex flex-col gap-4 rounded-xl border border-secondary bg-primary p-6">
                ...
                <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <h3 className="text-md font-semibold text-primary">Link Building Prospects</h3>
                            <p className="text-sm text-tertiary">Domains that link to your competitors but not to you — sorted by authority and relevance.</p>
                        </div>
                    </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="relative flex-1 min-w-[200px] max-w-sm">
                        ...
                        <input
                            type="text"
                            placeholder="Search domains or competitors..."
                            value={search}
                ...
                {showFilters && (
                    <div className="flex flex-wrap gap-3">
                        <FilterSelect label="Channel" value={channelFilter} options={CHANNEL_FILTERS} onChange={(v) => { setChannelFilter(v); resetPage(); }} />
                        ...
                        <FilterSelect label="Status" value={statusFilter} options={STATUS_FILTERS} onChange={(v) => { setStatusFilter(v); resetPage(); }} />
                    </div>
                )}
                ...
                {filteredData.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-12">
                        ...
                        <p className="text-sm font-medium text-primary mb-1">No prospects match your filters</p>
                                    ...
                                    <tr className="border-b border-secondary">
                                        {columnVisibility.domain && (
                                            <th className="cursor-pointer px-3 py-2.5 font-medium text-tertiary" onClick={() => handleSort("referringDomain")}>
                                                <div className="flex items-center gap-1">Domain <SortIcon column="referringDomain" /></div>
                                            </th>
                                        )}
                                        {columnVisibility.score && (
                                            <th className="cursor-pointer px-3 py-2.5 text-right font-medium text-tertiary" onClick={() => handleSort("prospectScore")}>
                                                <div className="flex items-center justify-end gap-1">Score <SortIcon column="prospectScore" /></div>
                                            </th>
                                        )}
                                        {columnVisibility.channel && (
                                            <th className="px-3 py-2.5 text-center font-medium text-tertiary">Channel</th>
                                        )}
                                        {columnVisibility.difficulty && (
                                            <th className="px-3 py-2.5 text-center font-medium text-tertiary">Difficulty</th>
                                        )}
                                        {columnVisibility.impact && (
                                            <th className="cursor-pointer px-3 py-2.5 text-right font-medium text-tertiary" onClick={() => handleSort("estimatedImpact")}>
                                                <div className="flex items-center justify-end gap-1">Impact <SortIcon column="estimatedImpact" /></div>
                                            </th>
                                        )}
                                        {columnVisibility.domainRank && (
                                            <th className="cursor-pointer px-3 py-2.5 text-right font-medium text-tertiary" onClick={() => handleSort("domainRank")}>
                                                <div className="flex items-center justify-end gap-1">DR <SortIcon column="domainRank" /></div>
                                            </th>
                                        )}
                                        {columnVisibility.compLinks && (
                                            <th className="cursor-pointer px-3 py-2.5 text-right font-medium text-tertiary" onClick={() => handleSort("linksToCompetitors")}>
                                                <div className="flex items-center justify-end gap-1">Comp Links <SortIcon column="linksToCompetitors" /></div>
                                            </th>
                                        )}
                                        {columnVisibility.competitors && (
                                            <th className="cursor-pointer px-3 py-2.5 text-left font-medium text-tertiary" onClick={() => handleSort("competitors")}>
                                                <div className="flex items-center gap-1">Competitors <SortIcon column="competitors" /></div>
                                            </th>
                                        )}
                                        {columnVisibility.status && (
                                            <th className="px-3 py-2.5 text-center font-medium text-tertiary">Status</th>
                                        )}
                                        {columnVisibility.actions && (
                                            <th className="px-3 py-2.5 text-center font-medium text-tertiary">Actions</th>
                                        )}
                                    </tr>
                                </thead>
                                <tbody>
                                            ...
                                            >
                                                ...
                                                {columnVisibility.channel && (
                                                    <td className="px-3 py-2.5 text-center">
                                                        <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${channel.bg} ${channel.text}`}>{channel.label}</span>
                                                    </td>
                                                )}
                                                {columnVisibility.difficulty && (
                                                    <td className="px-3 py-2.5 text-center">
                                                        <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${difficulty.bg} ${difficulty.text}`}>{difficulty.label}</span>
                                                    </td>
                                                )}
                                                {columnVisibility.impact && (
                                                    <td className="px-3 py-2.5 text-right">
                                                ...
                                                {columnVisibility.status && (
                                                    <td className="px-3 py-2.5 text-center">
                                                        <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${statusBadge.bg} ${statusBadge.text}`}>{statusBadge.label}</span>
                                                    </td>
                                                )}
                                                {columnVisibility.actions && (
                                                    <td className="px-3 py-2.5">
