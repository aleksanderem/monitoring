...
async function insertJob(
  ...
  return await t.run(async (ctx: any) => {
    return await ctx.db.insert("competitorContentGapJobs", {
      domainId,
      competitorId,
...
describe("competitorContentGapJobs", () => {
  describe("getActiveJobForCompetitor", () => {
    test("returns null when no jobs exist", async () => {
      ...
      const result = await t.query(
        api.competitorContentGapJobs.getActiveJobForCompetitor,
        { competitorId: compId }
      );
  ...
  describe("getJob", () => {
    test("returns job by ID", async () => {
      ...
      const result = await t.query(api.competitorContentGapJobs.getJob, {
        jobId,
      });
  ...
  describe("getJobInternal", () => {
    test("returns job by ID via internal query", async () => {
      ...
      const result = await t.query(
        internal.competitorContentGapJobs.getJobInternal,
        { jobId }
      );
  ...
  describe("updateJobInternal", () => {
    test("updates job status", async () => {
      ...
      await t.mutation(
        internal.competitorContentGapJobs.updateJobInternal,
    ...
    test("updates job to completed with opportunities count", async () => {
      const t = convexTest(schema, modules);
      const tenant = await createTenantHierarchy(t);
      const compId = await createCompetitor(t, tenant.domainId, "rival.com");
      const jobId = await insertJob(t, tenant.domainId, compId, {
      ...
      await t.mutation(
        internal.competitorContentGapJobs.updateJobInternal,
        { jobId, status: "completed", opportunitiesFound: 15, completedAt: 2000 }
      );
      ...
      expect(updated!.opportunitiesFound).toBe(15);
    ...
    test("updates job to failed with error", async () => {
      ...
      await t.mutation(
        internal.competitorContentGapJobs.updateJobInternal,
        { jobId, status: "failed", error: "Analysis timeout", completedAt: 3000 }
      );
      ...
      expect(updated!.error).toBe("Analysis timeout");
    });
  });
  ...
  describe("cancelJob", () => {
    test("cancels a pending job", async () => {
      ...
      await t.mutation(api.competitorContentGapJobs.cancelJob, { jobId });

      const job = await t.query(api.competitorContentGapJobs.getJob, { jobId });
    ...
    test("throws when cancelling a completed job", async () => {
      ...
      await expect(
        t.mutation(api.competitorContentGapJobs.cancelJob, { jobId })
