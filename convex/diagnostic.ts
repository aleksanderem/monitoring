...
async function buildDiagnosticSnapshot(ctx: QueryCtx, org: Doc<"organizations">) {
  ...
  const invariants: Array<{ name: string; status: "ok" | "warning" | "violation"; details: string }> = [];
  const now = Date.now();
  ...
  for (const team of teams) {
    ...
    for (const project of teamProjects) {
      ...
      for (const domain of domains) {
        ...
        // (This is what getPositionDistribution and getMonitoringStats use)
        let monTop3 = 0;
        let monTop10 = 0;
        let monTotalPos = 0;
        let monPosCount = 0;
        ...
        for (const kw of activeKws) {
          // Check denormalized currentPosition
          ...
          // Check recentPositions array
          const recent = kw.recentPositions ?? [];
          if (recent.length > 0) kwWithRecentPos++;

          // 7-day movement from recentPositions (same logic as getMonitoringStats)
          if (recent.length >= 2) {
          ...
          // Check actual keywordPositions table (ground truth)
          ...
          if (posRecords.length > 0) {
            ...
            // Find the latest position record by date
            const latestPos = posRecords.reduce((a, b) => a.date > b.date ? a : b);

            // True denormalization drift: latest position has a non-null value
            // but keyword.currentPosition is null. If latest position is null
            // (keyword dropped from SERP), then currentPosition: null is correct.
            if (cp == null && latestPos.position != null) {
              missingDenorm.push(kw.phrase);
            }

            // Staleness: positions exist but denormalized data is >48h old
        ...
        // (This is what getVisibilityStats uses)
        ...
        // ── Visibility history: movement data ──
        // (This is what getMovementTrend and getLatestVisibilityMetrics use)
        ...
        // 1. Monitoring says 0 positions but keyword positions table has data with non-null values
        ...
        // 2. Avg position shows 0 but there are keywords with actual position data
        ...
        // 3. Monitoring top3 vs visibility top3 mismatch
        ...
        // 4. Monitoring movement=0 but visibility shows movement
        ...
        // 6. recentPositions empty for keywords that have position history
        ...
        // 7. Position records exist with non-null values but recentPositions is empty (movement chart will be blank)
        ...
        // Push invariant for domains with contradictions
        ...
        // Content gap data quality: check for NaN scores
        ...
        // Content gap priority consistency: recalculate and compare
        ...
        // Backlink summary vs table record count
        ...
        // ── Insights (depends on on-site, backlinks-ext, link-building) ──
        ...
        // ── Cross-tab checks (depends on everything) ──
        ...
        // ── Collect all module contradictions into the main array ──
        ...
        // Update contradiction invariant count if new contradictions found
        ...
        domainStats.push({
          ...
          crossValidation: {
            monitoring: {
              top3: monTop3,
              top10: monTop10,
              avgPosition: monAvgPos,
              totalWithPosition: monPosCount,
              gainers7d: monGainers,
              losers7d: monLosers,
            },
            visibility: {
              top3: visTop3,
              top10: visTop10,
              avgPosition: visAvgPos,
              totalKeywords: rankedDiscovered.length,
              gainers: visGainers,
              losers: visLosers,
            },
  ...
  if (!invariants.some((i) => i.name === "limit_exceeded")) {
    invariants.push({ name: "limit_enforcement", status: "ok", details: "All domains within keyword limits" });
  }
  ...
  if (!invariants.some((i) => i.name === "cross_view_contradiction")) {
    invariants.push({ name: "cross_view_consistency", status: "ok", details: "No contradictions between monitoring and visibility data" });
  }
...
// ─── Public query (requires auth + superAdmin) ────
